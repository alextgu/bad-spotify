...
.venv/bin/pytest -q                        # 238 tests, ~10 s on a laptop
PYTHONPATH=src .venv/bin/python run.py --source replay --ticks 8 --no-hud
```

- The test count is regenerable: `.venv/bin/pytest --collect-only -q` → `238 tests collected`.
...
(`src/badspotify/capture/gate.py`); 238 tests of which roughly 87% assert
